...
// EnhancedTaskManager provides advanced task management with job queuing and worker pools
...
func NewWorker(id int, taskMgr *EnhancedTaskManager) *Worker {
...
// NewEnhancedTaskManager creates a new enhanced task manager
func NewEnhancedTaskManager(workerCount int, kubevirtClient *kubevirt.Client) *EnhancedTaskManager {
...
func (tm *EnhancedTaskManager) startWorkers() {
	tm.workerMutex.Lock()
...
func (tm *EnhancedTaskManager) jobDispatcher() {
...
func (tm *EnhancedTaskManager) getAvailableWorker() *Worker {
...
func (tm *EnhancedTaskManager) CreateTask(name, namespace, vmName, mediaID, imageURL string) string {
...
func (tm *EnhancedTaskManager) GetTask(taskID string) (*TaskInfo, bool) {
	tm.taskMutex.RLock()
...
func (tm *EnhancedTaskManager) UpdateTaskState(taskID, state, status string, message string) error {
	tm.taskMutex.Lock()
	defer tm.taskMutex.Unlock()
...
func (tm *EnhancedTaskManager) UpdateTaskProgress(taskID, message string) error {
	tm.taskMutex.Lock()
	defer tm.taskMutex.Unlock()
...
func (tm *EnhancedTaskManager) CompleteTask(taskID, finalMessage string) error {
	return tm.UpdateTaskState(taskID, redfish.TaskStateCompleted, "OK", finalMessage)
}

// FailTask marks a task as failed
func (tm *EnhancedTaskManager) FailTask(taskID, errorMessage string) error {
	return tm.UpdateTaskState(taskID, redfish.TaskStateException, "Warning", errorMessage)
}

// updateStats updates task statistics
func (tm *EnhancedTaskManager) updateStats(duration time.Duration, success bool) {
	tm.statsMutex.Lock()
...
func (tm *EnhancedTaskManager) updateQueueStats(delta int64) {
	tm.statsMutex.Lock()
	defer tm.statsMutex.Unlock()
...
func (tm *EnhancedTaskManager) GetStats() map[string]interface{} {
	tm.statsMutex.RLock()
...
func (tm *EnhancedTaskManager) CleanupOldTasks(maxAge time.Duration) {
	tm.taskMutex.Lock()
	defer tm.taskMutex.Unlock()
...
func (tm *EnhancedTaskManager) StartCleanupRoutine() {
...
func (tm *EnhancedTaskManager) Stop() {
